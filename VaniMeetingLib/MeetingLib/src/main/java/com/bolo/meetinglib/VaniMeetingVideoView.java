...
public class VaniMeetingVideoView extends ViewGroup {
    /**
    ...
    private final RendererCommon.RendererEvents rendererEvents
        ...
        public void onFirstFrameRendered() {
            VaniMeetingVideoView.this.onFirstFrameRendered();
        }
        ...
        public void onFrameResolutionChanged(
                ...
                int rotation) {
            VaniMeetingVideoView.this.onFrameResolutionChanged(
    ...
    public VaniMeetingVideoView(Context context) {
        super(context);
...
//        setMirror(false);
    ...
    public VaniMeetingVideoView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context  = context;
        surfaceViewRenderer = new SurfaceViewRenderer(context);
        addView(surfaceViewRenderer);
...
//        setMirror(false);
    ...
    public VaniMeetingVideoView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public VaniMeetingVideoView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    public  void setVideoTrack(Track videoTrack)
    {
        if(videoTrack.track != null){
...
//        }
    ...
    private void onFirstFrameRendered() {
        post(() -> {
            Log.d("", "First frame rendered.");
            surfaceViewRenderer.setBackgroundColor(Color.RED);
        });
    }
    ...
    private void removeRendererFromVideoTrack() {
        if (rendererAttached) {
            ...
            Log.e("removeRendererFromVideoTrack","removeRendererFromVideoTrack");
            MeetingHandler.getInstance().removeEventListner("onTrackEnded", eventEmitterHandler);
            MeetingHandler.getInstance().removeEventListner("onEnded", eventEmitterHandler);
...
//    }
    ...
    EventEmitterHandler eventEmitterHandler = new EventEmitterHandler() {
        @Override
        public void onEvent(String type ,  Object data) {
            if(type.equalsIgnoreCase("onTrackEnded")){
                onTrackEnded((Track) data);
            }
            else{
                removeRendererFromVideoTrack();
    ...
    private void subScribeToEnd(){
        MeetingHandler.getInstance().addEventListner("onTrackEnded", eventEmitterHandler);
        MeetingHandler.getInstance().addEventListner("onEnded", eventEmitterHandler);

    }

    private void onTrackEnded(Track track){
        if(track != null && track.trackId .equalsIgnoreCase( this.track.trackId)){
            Log.e("OnTrackEnded" , "OnTrackEnded ");
